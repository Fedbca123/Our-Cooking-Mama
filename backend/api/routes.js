		...
		if (req.file != null) {
			...
			cloudinary.v2.uploader.upload(
				...
				async (err, result) => {
					...
					const userId = req.body.userId;
					const validAccount = await userRegister
						.findOne({ _id: userId })
						.exec();
					try {
...
router.post("/editPost", upload.single("file"), function (req, res) {
	try {
		cloudinary.v2.uploader.upload(req.file.path, async (err, result) => {
			...
			const profileId = req.body.ProfileID;
			const validRecipe = await userPost
	...
	try {
		...
		if (!findingPost) {
			...
			if (findingPost.ProfileID == profileId) {
				...
				var ret = {
					id: -1,
					error: "You cannot delete this post! UserIDs do not match!",
				};
				return res.json(ret);
			}
		}
...
router.post("/getMainFeed", async (req, res) => {
	...
	try {
		...
		if (result == null) {
			...
			// get all posts for every user in following array
			console.log("Following: " + result.Following);
			const followingPosts = result.Following.map(
				async (influencerID) => {
					try {
						userPosts = await userPost
							.find({ ProfileID: influencerID })
							.sort({ _id: -1 })
							.exec();
						return userPosts;
					} catch (error) {
...
router.post("/deleteComment", async (req, res) => {
	...
	try {
		const deleteComment = await userComment
...
router.post("/follow", async (req, res) => {
	...
	const resultFollower = await userProfile
		.findOne({ UserID: influencerID })
		.exec();
	const resultFollowing = await userProfile
		.findOne({ UserID: creeperID })
		.exec();
	try {
		...
		if (resultFollowing == null) {
			var ret = {
				...
				error:
					"User Profile: " +
					creeperID +
					" To Be Following Not Found.",
			};
			return res.json(ret);
		}

		// Pull the following document for creeper --> influencer
		const followingDocument = await following
			.findOne({ ProfileID: creeperID })
			.exec();
		console.log(
			"Following (creeper --> influencer) Document: " + followingDocument,
		);
		if (followingDocument != null) {
			...
			if (followingDocument.Following.includes(influencerID)) {
				// if so, gtfo
				var ret = {
					error: "User: " + influencerID + " Already Followed.",
				};
				return res.json(ret);
			} else {
		...
		const followerDocument = await followers
			.findOne({ ProfileID: influencerID })
			.exec();
		console.log(
			"Follower (creeper <-- influencer) Document: " + followerDocument,
		);
		if (followerDocument != null) {
			...
			if (followerDocument.Followers.includes(creeperID)) {
				// if so, gtfo
				var ret = {
					error: "User: " + creeperID + " Already Follower.",
				};
				return res.json(ret);
			} else {
...
router.post("/addLike", async (req, res) => {
	...
	try {
		const like = await postLikes
			.findOne({
				$and: [{ PostID: data.PostID }, { LikersID: data.LikersID }],
			})
			.exec();
...
router.post("/removeLike", async (req, res) => {
	...
	try {
		const like = postLikes.find({
			$and: [{ PostID: postID }, { LikersID: likersID }],
		});
		if (like != "") {
			...
			else res.status(200).json({ LikeRemoved: true, error: "" });
		} else
			res.status(400).json({
				error: "You have not liked this post yet.",
			});
